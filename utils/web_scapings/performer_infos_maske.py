...
from config import PROJECT_PATH, RASSE_JSON, IAFD_INFOS_JSON_PATH
...
class PerformerInfosMaske():
    ...
    def set_nations_in_labels(self, nations, infos_webside):
        ...
        for zahl, nation_ger in enumerate(nations):
            ...
            infos_webside.set_tooltip_text(f"lbl_performer_nation_{zahl+1}", "", f"Datenbank: {nation_ger}", "Datenbank")
    ...
    def get_artistdata_from_ui(self, database: bool=False) -> Tuple[dict, list]:
        ...
        daten_satz = {
            ...
            "Rassen": rassen,
            "Nation": nations,            
            "Geburtstag": self.Main.lnEdit_performer_geburtstag.text().strip(),
            "Birth_Place": self.Main.lnEdit_performer_geburtsort.text().strip(),
            "OnlyFans": self.get_social_media_from_buttons(),
            "Boobs": self.Main.lnEdit_performer_boobs.text().strip(),
            "Gewicht": int(self.Main.lnEdit_performer_gewicht.text() or 0),
            "Groesse": int(self.Main.lnEdit_performer_groesse.text() or 0),
            "Bodytyp": self.Main.lnEdit_performer_bodytyp.text().strip(),
            ...
            "Haarfarbe": self.Main.lnEdit_performer_haar.text().strip(),
            "Augenfarbe": self.Main.lnEdit_performer_augen.text().strip(),
            "Aktiv": self.Main.lnEdit_performer_aktiv.text().strip(),     }
        names_link_satz = []
        if database:            
    ...
    def update_nations(nations_ger: str, artist_id: int, datenbank_darsteller) -> str:
        nation_message=""
        nations_ids: list = []
        for nation_german in nations_ger.split(", "):
            nations_ids.append(datenbank_darsteller.get_nations_id_from_nations_ger(nation_german))
        errview, is_addet = datenbank_darsteller.update_or_add_nations_datensatz(nations_ids, artist_id)
    ...
    def update_rassen(rassen_ger: str, artist_id: int, datenbank_darsteller) -> str:
        rassen_message: str = ""
        rassen_ids: list = []
        for rasse_german in rassen_ger.split("/"):
            rassen_ids.append(datenbank_darsteller.get_rassenids_from_rasse_german(rasse_german))
        ...
        if errview and not "Rasse: None nicht in der Datenbank !":
            ...
            rassen_message=".➕Eine Rasse wurde geaddet"
    ...
    def update_datensatz(self):
        ...
        daten_satz_ui, names_link_satz_ui, nation_message=self.get_artistdata_from_ui(database=True)                 
        ...
        if errview:
            ...
            for widget in self.Main.performers_tab_widgets(): # masken farbe wieder bereinigen bei erfolgreichen update
        ...
        message= f"Update {artist_id}: {datensatz_msg}{nameslink_msg}{iafd_message}"
    ...
    def update_and_check_datensatz_for_db(self, datensatz_ui, artist_id, datenbank_darsteller):
        ...
        if datensatz_db[0]:
            ...
            rassen_ids = self.translate_rassen_ger_in_number(datensatz_ui["Rassen"], datenbank_darsteller)
            errview, _ = datenbank_darsteller.update_or_add_rassen_datensatz(rassen_ids, artist_id)
    ...
    def update_tabelle(self):
        ...
        for zeile in range(table_performer_count):            
            ...
            if Path(IAFD_INFOS_JSON_PATH).exists() and json.loads(IAFD_INFOS_JSON_PATH.read_bytes()):           
    ...
    def names_link_from_iafd(self, ordner, perfid, iafd_link, artist_id, datenbank_darsteller):
        if not Path(IAFD_INFOS_JSON_PATH).exists(): # get iafd_infos from ui
            iafd_infos = {"alias": self.Main.lnEdit_IAFD_artistAlias.text(),
                          "image_pfad": f"__artists_Images/{ordner}/[IAFD]-{perfid}.jpg"}
        else:
            iafd_infos = json.loads(IAFD_INFOS_JSON_PATH.read_bytes())
        ...
        if old_image_pfad:
            ...
            json.dump(iafd_infos,open(IAFD_INFOS_JSON_PATH,'w'),indent=4, sort_keys=True)
    ...
    def set_iafd_infos_in_ui(self):
        if not Path(IAFD_INFOS_JSON_PATH).exists():
        ...
        infos_webside=Infos_WebSides(MainWindow=self.Main)
        if nations_ger:
            self.set_nations_in_labels(nations_ger, infos_webside)
        # --------- Haarfarbe ------------------- #
        hair_color=iafd_infos.get("Haarfarbe")
        if hair_color:
            self.Main.lnEdit_performer_haar.setText(hair_color)
        # --------- Gewicht ------------------- #
        gewicht=iafd_infos.get("Gewicht")
        if gewicht:
            self.Main.lnEdit_performer_gewicht.setText(f"{gewicht}")
        # --------- Körper-Größe ------------------- #
        groesse=iafd_infos.get("Groesse")
        if groesse:
            self.Main.lnEdit_performer_groesse.setText(f"{groesse}")
        # --------- Geburts-Ort ------------------- #
        geburtsort=iafd_infos.get("Birth_Place")
        if geburtsort:
            self.Main.lnEdit_performer_geburtsort.setText(f"{geburtsort}")
        # --------- Geburts-Tag ------------------- #
        geburtstag=iafd_infos.get("Geburtstag")
        if geburtstag:
            self.Main.lnEdit_performer_geburtstag.setText(f"{geburtstag}") 
        # --------- Geburts-Tag ------------------- #
        piercing=iafd_infos.get("Piercing")
        if piercing:
            self.Main.txtEdit_performer_piercing.setPlainText(piercing)
        # --------- Geburts-Tag ------------------- #
        tattoo=iafd_infos.get("Tattoo")
        if tattoo:
            self.Main.txtEdit_performer_tattoo.setPlainText(tattoo)
        # --------- Geburts-Tag ------------------- #
        aktiv=iafd_infos.get("Aktiv")          
        self.Main.lnEdit_performer_aktiv.setText(aktiv) 
        # --------- boobs ------------------- #
        boobs=iafd_infos.get("Boobs")
        if boobs:
            self.Main.lnEdit_performer_boobs.setText(boobs)
        # --------- IAFD Image ---------------- #
